...
await upsert(
	...
	questions.map(
		({
			...
			prompt,
			choices: choices ?? [],
			choice_order_strategy: choice_order_strategy ?? 'shuffle',
			fixed_choice_ids: fixed_choice_ids ?? [],
			...
			math_template: math_template ?? null,
			math_match_mode: math_match_mode ?? 'normalized',
			accepted_math_answers: accepted_math_answers ?? [],
