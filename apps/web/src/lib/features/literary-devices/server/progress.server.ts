...
import type { LiteraryDevicesPathItem, LiteraryDevicesPathItemProgress } from '../types';
...
export async function getActivityHistory(client: SupabaseClient, userId: string): Promise<ActivityHistoryItem[]> {
	const { data, error } = await client
...
export async function getPathItemProgress(
...
): Promise<LiteraryDevicesPathItemProgress[]> {
	...
	const attemptsByQuiz = new Map<string, { latest_score: number | null; best_score: number | null; total_attempts: number }>();
	for (const attempt of attempts ?? []) {
		const key = `${attempt.quiz_id}@${attempt.quiz_version}`;
		const current = attemptsByQuiz.get(key) ?? { latest_score: null, best_score: null, total_attempts: 0 };
	...
	return pathItems.map((item) => {
		const quizStats = item.item_type === 'quiz' ? attemptsByQuiz.get(`${item.item_id}@${item.item_version}`) : undefined;
		const completed =
...
function summarizeDeviceStats(answers: { device: string; is_correct: boolean }[]) {
...
export async function getDeviceStats(client: SupabaseClient, attemptIds: string[]): Promise<DeviceStat[]> {
	...
	return summarizeDeviceStats(data ?? []);
}
