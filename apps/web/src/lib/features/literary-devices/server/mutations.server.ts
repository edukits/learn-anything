import type { SupabaseClient } from '@supabase/supabase-js';
import { LITERARY_DEVICES_TOPIC_ID } from './content.server';
import type { ContentRelease, LessonVersion, PracticeQuizQuestion, QuizVersion } from '../types';

type CompleteLessonResult = {
	activity_event_id: string;
	xp_awarded: number;
};

type SubmittedAnswer = {
	questionId: string;
	selectedChoiceId: string;
};

type SubmitQuizParams = {
	userId: string;
	release: ContentRelease;
	quiz: QuizVersion;
	questions: PracticeQuizQuestion[];
	answers: SubmittedAnswer[];
	submissionKey: string;
};

type SubmitQuizResult = {
	attemptId: string;
	xpAwarded: number;
};

type RpcAnswer = {
	question_id: string;
	selected_choice_id: string;
};

type CompleteReviewSessionParams = {
	userId: string;
	release: ContentRelease;
	reviewSessionId: string;
	questions: PracticeQuizQuestion[];
	answers: SubmittedAnswer[];
	submissionKey: string;
};

export class LearnerMutationError extends Error {
	constructor(
		message: string,
		readonly status = 400
	) {
		super(message);
		this.name = 'LearnerMutationError';
	}
}

function buildRpcAnswers(questions: PracticeQuizQuestion[], answers: SubmittedAnswer[], label: string): RpcAnswer[] {
	const answersByQuestion = new Map(answers.map((answer) => [answer.questionId, answer.selectedChoiceId]));

	if (answersByQuestion.size !== questions.length) {
		throw new LearnerMutationError(`Answer every question before submitting the ${label}.`);
	}

	return questions.map((question) => {
		const selectedChoiceId = answersByQuestion.get(question.question_id);
		const validChoiceIds = new Set(question.choices.map((choice) => choice.id));

		if (!selectedChoiceId || !validChoiceIds.has(selectedChoiceId)) {
			throw new LearnerMutationError(`Invalid answer for ${question.question_id}.`);
		}

		return {
			question_id: question.question_id,
			selected_choice_id: selectedChoiceId
		};
	});
}

export async function completeLesson(
	client: SupabaseClient,
	userId: string,
	release: ContentRelease,
	lesson: LessonVersion
): Promise<CompleteLessonResult> {
	const { data, error } = await client.rpc('complete_lesson', {
		p_user_id: userId,
		p_topic_area_id: LITERARY_DEVICES_TOPIC_ID,
		p_release_id: release.id,
		p_lesson_id: lesson.lesson_id,
		p_lesson_version: lesson.version
	});

	if (error) {
		throw new Error(error.message);
	}

	const [result] = data ?? [];
	return {
		activity_event_id: result?.activity_event_id ?? '',
		xp_awarded: result?.xp_awarded ?? 0
	};
}

export async function submitQuiz(
	client: SupabaseClient,
	{ userId, release, quiz, questions, answers, submissionKey }: SubmitQuizParams
): Promise<SubmitQuizResult> {
	if (!submissionKey) {
		throw new LearnerMutationError('Missing quiz submission key.');
	}

	const rpcAnswers = buildRpcAnswers(questions, answers, 'quiz');

	const { data, error } = await client.rpc('submit_quiz', {
		p_user_id: userId,
		p_topic_area_id: LITERARY_DEVICES_TOPIC_ID,
		p_release_id: release.id,
		p_quiz_id: quiz.quiz_id,
		p_quiz_version: quiz.version,
		p_answers: rpcAnswers,
		p_submission_key: submissionKey
	});

	if (error) {
		throw new LearnerMutationError(error.message, 500);
	}

	const [result] = data ?? [];
	if (!result?.attempt_id) {
		throw new LearnerMutationError('Quiz submission did not return an attempt.', 500);
	}

	return {
		attemptId: result.attempt_id,
		xpAwarded: result.xp_awarded ?? 0
	};
}

export async function completeReviewSession(
	client: SupabaseClient,
	{ userId, release, reviewSessionId, questions, answers, submissionKey }: CompleteReviewSessionParams
): Promise<SubmitQuizResult> {
	if (!submissionKey) {
		throw new LearnerMutationError('Missing review submission key.');
	}

	const rpcAnswers = buildRpcAnswers(questions, answers, 'review');

	const { data, error } = await client.rpc('complete_review_session', {
		p_user_id: userId,
		p_topic_area_id: LITERARY_DEVICES_TOPIC_ID,
		p_release_id: release.id,
		p_review_session_id: reviewSessionId,
		p_answers: rpcAnswers,
		p_submission_key: submissionKey
	});

	if (error) {
		throw new LearnerMutationError(error.message, 500);
	}

	const [result] = data ?? [];
	if (!result?.attempt_id) {
		throw new LearnerMutationError('Review submission did not return an attempt.', 500);
	}

	return {
		attemptId: result.attempt_id,
		xpAwarded: result.xp_awarded ?? 0
	};
}
