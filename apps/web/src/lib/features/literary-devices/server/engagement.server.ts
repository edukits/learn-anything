...
function zonedTimeToUtc(year: number, month: number, day: number, hour: number, minute: number, second: number, ms = 0) {
...
export async function getEngagementSummary(client: SupabaseClient, userId: string): Promise<EngagementSummary> {
	const today = zonedDayBounds();
	const [{ data: goal, error: goalError }, { data: streak, error: streakError }, xpSummary] =
		await Promise.all([
			client.from('daily_goal_settings').select('user_id,daily_xp_goal').eq('user_id', userId).maybeSingle(),
			client
				.from('streaks')
