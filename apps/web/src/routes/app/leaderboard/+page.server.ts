...
export const load: PageServerLoad = async ({ locals, parent, url }) => {
	...
	const activeTopicId =
		requestedTopicId && activeEnrollments.some((enrollment) => enrollment.topic_area_id === requestedTopicId)
			? requestedTopicId
			: null;
...
export const actions: Actions = {
	profile: async ({ request, locals }) => {
		...
		try {
			...
			return fail(400, {
				error: error instanceof Error ? error.message : 'Unable to save leaderboard profile.',
